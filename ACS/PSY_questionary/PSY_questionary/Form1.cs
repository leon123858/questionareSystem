...
namespace PSY_questionary
{
    public partial class Form1 : Form
    {
        ...
        //備分
        void backupOne(MongoClient mongoClient, MongoClient mongoClient_local, string db, string table_name)
        {
            var collection = mongoClient.GetDatabase(db).GetCollection<BsonDocument>(table_name);
            var documents = collection.Find(new BsonDocument()).ToList();
            //載入在地
            var table = mongoClient_local.GetDatabase(db).GetCollection<BsonDocument>(table_name);
        ...
        private void backup_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                progressBar.Value = 0;
                //獲取雲資料
                MongoClient dbClient_cloud = new MongoClient("mongodb+srv://leon1234858:8ntscpal@cluster0.gyixj.gcp.mongodb.net/?retryWrites=true&w=majority");
                MongoClient dbClient_local = new MongoClient("mongodb://127.0.0.1:27017");
                progressBar.Value += progressBar.Step * 10;
                for (int i = 0; i < 9; i++)
                {
                    backupOne(dbClient_cloud, dbClient_local, "qn", "qn" + i);
                    progressBar.Value += progressBar.Step * 10;
                }
                backupOne(dbClient_cloud, dbClient_local, "personq", "personall");
                progressBar.Value = 100;
                setShow("完成資料備份");
                dataTransform.Enabled = true;
            }
            catch (Exception et)
            {
                setShow(et.Message);
        ...
        private void transformOne(MongoClient mongoClient, int q)
        {
            var collection = mongoClient.GetDatabase("qn").GetCollection<BsonDocument>("qn" + q);
            var documents = collection.Find(new BsonDocument()).ToList();
            if (documents.Count > 0)
            {
                //寫下SQL指令
        ...
        private void showOnDatagrid_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                progressBar.Value += progressBar.Step * 20;
                cn.ConnectionString = @"Data Source=(LocalDB)\MSSQLLocalDB;" +
                    "AttachDbFilename=|DataDirectory|Database.mdf;" +
                    "Integrated Security=True";
                SqlDataAdapter daEmp = new SqlDataAdapter
                    ("SELECT * FROM page" + show_which_qn.Value.ToString(), cn);
                progressBar.Value += progressBar.Step * 20;
                daEmp.Fill(ds, "page" + show_which_qn.Value.ToString());
                dataGridView.DataSource = ds.Tables["page" + show_which_qn.Value.ToString()];
                progressBar.Value += progressBar.Step * 20;
                show.Text += "資料顯示第" + show_which_qn.Value.ToString() + "成功\r\n";
                outCSV_one.Enabled = true;
                outCSV_all.Enabled = false;
                progressBar.Value += progressBar.Step * 40;
            }
            catch (Exception et)
            {
                setShow(et.Message);
        ...
        private void outCSV_all_Click(object sender, EventArgs e)
        {
            ...
            strValue = getOutPut(strValue);
            //存成檔案（注意！！當有中文字的時候 存檔案一定要用 UTF8）
            string strFile = input_date2 + "/allpage" + show_which_qn.Value.ToString() + ".csv";
            try
            {
                if (!string.IsNullOrEmpty(strValue))
                {
