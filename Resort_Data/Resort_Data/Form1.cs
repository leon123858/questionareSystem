...
namespace Resort_Data
{
    public partial class Form1 : Form
    {
        ...
        private void Read_Click(object sender, EventArgs e)
        {
            ...
            int number = Int32.Parse(count.Value.ToString());
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    arrayList.Add(i.Split('_'));
                }
                //MessageBox.Show(str);
                for (var i = 0; i < number; i++)
                {
                    using (StreamWriter sw = new StreamWriter(i.ToString() + ".csv",true, Encoding.Default))    
                    {
                        foreach (string[] j in arrayList)
                        {
                            if (j[0] == i.ToString())
                            {
                                var tmp = "";
        ...
        private void now_data_Click(object sender, EventArgs e)
        {
            ...
            p.Start();
            p.StandardInput.WriteLine("explorer " + System.Windows.Forms.Application.StartupPath);
        }

        private void add_header_Click(object sender, EventArgs e)
        {
            int number = Int32.Parse(count.Value.ToString());
            if (number == 9)
            {
                for (var i = 0; i < number; i++)
                {
                    using (StreamWriter sw = new StreamWriter(i.ToString() + ".csv", true, Encoding.Default))
                    {
                        sw.WriteLine("測試者編號," + csv_title[i]);
                    }
                }
                MessageBox.Show("Save successful");
            }
            else
                MessageBox.Show("添加標頭功能只適用於9張問卷, 請修改原代碼後再作使用");
        }
    }
}
